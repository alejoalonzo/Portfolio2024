...
@Component({
  ...
  styleUrl: './text-three.component.scss'
})
export class TextThreeComponent implements OnInit {
  ...
  preload = (): void => {
    var container = this.magic.nativeElement;
    ...
    const font = loader.load(
      ...
      "assets/fonts/Lobster_Regular.json",
...
@Component({
  selector: 'app-text-three-Environment',
  templateUrl: './text-three.component.html',
...
@Component({
  selector: 'app-text-three-CreateParticles',
  templateUrl: './text-three.component.html',
  styleUrls: ['./text-three.component.scss']
})
...
//     (geometry.boundingBox.max.y - geometry.boundingBox.min.y) / 2.85;
    ...
    for (let x = 0; x < shapes.length; x++) {
      ...
      points.forEach((element: { x: number; y: number }, z: any) => {
